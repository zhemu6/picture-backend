...
import com.lushihao.picture.domain.picture.valueobject.PictureReviewsStatusEnum;
import com.lushihao.picture.domain.user.entity.User;
import com.lushihao.picture.infrastructure.api.CosManager;
import com.lushihao.picture.infrastructure.api.aliyunai.AliYunAiApi;
import com.lushihao.picture.infrastructure.api.aliyunai.model.CreateCommonSynthesisTaskRequest;
...
import com.lushihao.picture.infrastructure.util.ColorSimilarUtils;
import com.lushihao.picture.infrastructure.util.SSLUtil;
...
import com.lushihao.picture.infrastructure.manager.upload.PictureUploadTemplate;
import com.lushihao.picture.infrastructure.manager.upload.UrlPictureUpload;
...
//        picture.setPicColor((uploadPictureResult.getPicColor()));
        ...
        transactionTemplate.execute(status -> {
            ...
            if (finalSpaceId != null) {
                boolean update = spaceApplicationService.addUpdatePicture(finalSpaceId, picture.getPicSize());
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        boolean idUpdate = pictureRepository.updateById(newPicture);
...
//            String fileUrl = element.attr("src");
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        Long count = pictureRepository.lambdaQuery().eq(Picture::getUrl, oldPictureUrl).count();
        if (count > 1) {
            return;
        }
        // 删除压缩以后的图片
    ...
    public void checkPictureAuth(User loginUser, Picture picture) {
        ...
        if (spaceId == null) {
            ...
            ThrowUtils.throwIf(!Objects.equals(loginUserId, picture.getUserId()) && !loginUser.isAdmin(), ErrorCode.NO_AUTH_ERROR, "您没有权限");
        } else {
            // 私有空间 仅空间管理员可以操作
    ...
    public void deletePicture(DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        transactionTemplate.execute(status -> {
            ...
            if (spaceId != null) {
                ...
                boolean update = spaceApplicationService.deleteUpdatePicture(spaceId, oldPicture.getPicSize());
                ThrowUtils.throwIf(!update, ErrorCode.OPERATION_ERROR, "额度更新失败");
            }
            return true;
        });
...
//        this.checkPictureAuth(loginUser, oldPicture);
        ...
        boolean isEdit = pictureRepository.updateById(picture);
    ...
    public CreateTaskResponse createPictureOutPaintingTask(CreatePictureOutPaintingTaskRequest createPictureOutPaintingTaskRequest, HttpServletRequest request) {
        ...
        Picture picture = Optional.ofNullable(pictureRepository.getById(pictureId)).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_ERROR, "图片不存在"));
        // 创建一个请求参数
    ...
    public Page<Picture> page(Page<Picture> picturePage, LambdaQueryWrapper<Picture> lambdaQueryWrapper) {
        return pictureRepository.page(picturePage, lambdaQueryWrapper);
    }
